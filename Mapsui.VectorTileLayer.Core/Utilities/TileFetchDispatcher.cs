...
namespace Mapsui.VectorTileLayer.Core.Utilities
{
    public class TileFetchDispatcher<T> : IFetchDispatcher, INotifyPropertyChanged
    {
        private FetchInfo? _fetchInfo;
        private readonly object _lockRoot = new object();
        private bool _busy;
        private bool _viewportIsModified;
        private readonly ITileCache<T> _tileCache;
        ...
        private readonly Func<TileInfo, T> _fetchTileAsFeature;

        public TileFetchDispatcher(
            ITileCache<T> tileCache,
            ITileSchema? tileSchema,
            Func<TileInfo, T> fetchTileAsFeature,
        ...
        private void FetchOnThread(TileInfo tileInfo)
        {
            ...
            {
                FetchCompleted(tileInfo, default(T), exception);
            }
        }
        ...
        private void FetchCompleted(TileInfo tileInfo, T feature, Exception? exception)
        {
            lock (_lockRoot)
            {
                if (exception == null)
